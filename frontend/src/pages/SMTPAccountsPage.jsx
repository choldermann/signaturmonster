...
const inputStyle = {
  ...
  border: "1px solid var(--border-3)", borderRadius: 8, color: "var(--text-2)",
...
function AccountForm({ initial, onSave, onCancel, toast }) {
  ...
  async function save() {
    if (!form.name.trim() || !form.relay_host.trim()) {
      toast("err", t("smtp.requiredNameAndHost"));
      return;
    }
    setSaving(true);
    try {
      ...
      if (r.id) { toast("ok", isNew ? t("smtp.accountCreated") : t("smtp.accountSaved")); onSave(); }
      else       { toast("err", r.detail || t("common.saveError")); }
  ...
  async function testConn() {
    ...
    try {
      ...
      if (id) {
        ...
        setTestResult(r);
        toast(r.ok ? "ok" : "err", r.ok ? t("smtp.testMailSent") : r.error);
      }
  ...
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--accent-bd)", borderRadius: 12, padding: "20px 24px", marginBottom: 16 }}>
      <div style={{ fontSize: 14, fontWeight: 700, color: "var(--accent)", marginBottom: 20 }}>
        {isNew ? t("smtp.newAccount") : `${t("smtp.editAccount")}: ${initial.name}`}
      </div>

      {/* Schnellauswahl */}
      <div style={{ marginBottom: 18 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text-4)", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 8 }}>{t("smtp.quickSelect")}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field label={t("smtp.nameInternal")}>
          <input style={inputStyle} value={form.name} onChange={e => u("name", e.target.value)} placeholder={t("smtp.namePlaceholder")} />
        </Field>
        <Field label={t("smtp.domainMatch")} hint={t("smtp.domainMatchHint")}>
          <input style={inputStyle} value={form.match_domain} onChange={e => u("match_domain", e.target.value.toLowerCase())} placeholder={t("smtp.domainMatchPlaceholder")} />
        </Field>
        ...
        <Field label={t("common.password")}>
          <div style={{ position: "relative" }}>
            ...
            <button onClick={() => setShowPass(s => !s)}
              style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "var(--text-5)", cursor: "pointer" }}>
              <Icon name={showPass ? "eye-off" : "eye"} size={15} />
            </button>
          </div>
        </Field>
      </div>
      <Field label={t("smtp.fromAddress")} hint={t("smtp.fromAddressHint")}>
        <input style={{ ...inputStyle }} value={form.from_address} onChange={e => u("from_address", e.target.value)} placeholder="info@firma.de" />
      </Field>
      <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--text-3)", cursor: "pointer", marginBottom: 20 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <button style={btnPrimary} onClick={save} disabled={saving}>
          <Icon name="device-floppy" size={15} />{saving ? t("common.saving") : t("common.save")}
        </button>
        ...
        {!isNew && (
          <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button style={{ ...btnSecondary, fontSize: 12 }} onClick={testConn} disabled={testing || !testAddr}>
              <Icon name={testing ? "loader" : "send"} size={14} />{testing ? t("smtp.sending") : t("smtp.testMail")}
            </button>
          </div>
        )}
      </div>
      ...
      {testResult && (
        <div style={{ marginTop: 12, padding: "10px 14px", borderRadius: 8,
          ...
          <div style={{ fontSize: 13, fontWeight: 600, color: testResult.ok ? "var(--green)" : "var(--red)", display: "flex", alignItems: "center", gap: 6 }}>
            <Icon name={testResult.ok ? "circle-check" : "circle-x"} size={15} />
            {testResult.ok ? testResult.message : t("common.error")}
          </div>
          {!testResult.ok && <div style={{ fontSize: 12, color: "var(--red-s)", marginTop: 5, fontFamily: "monospace" }}>{testResult.error}</div>}
        </div>
      )}
    </div>
  );
...
export default function SMTPAccountsPage({ toast }) {
  ...
  async function del(id) {
    ...
    toast("ok", t("smtp.accountDeleted"));
    load();
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "var(--text-1)", margin: 0 }}>{t("smtp.title")}</h1>
          <p style={{ fontSize: 13, color: "var(--text-5)", marginTop: 6 }}>
            {t("smtp.subtitle")}
          </p>
        </div>
        <button style={btnPrimary} onClick={() => setEditing("new")}>
          <Icon name="plus" size={15} />{t("smtp.newAccount")}
        </button>
      </div>

      {/* Info-Banner */}
      <div style={{ padding: "12px 16px", background: "var(--bg-input)", border: "1px solid var(--blue-bd)", borderRadius: 10, marginBottom: 20, display: "flex", gap: 12, alignItems: "flex-start" }}>
        <Icon name="info-circle" size={16} style={{ color: "var(--blue)", marginTop: 1, flexShrink: 0 }} />
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "var(--blue)" }}>{t("smtp.autoRelayTitle")}</div>
          <div style={{ fontSize: 12, color: "var(--text-5)", marginTop: 3, lineHeight: "18px" }}>
            {t("smtp.autoRelayDesc")} <strong style={{ color: "var(--text-3)" }}>{t("smtp.autoRelayOrder")}</strong> {t("smtp.autoRelayOrderDesc")}<br />
            {t("smtp.multiDomainHint")}
          </div>
        </div>
      </div>
      ...
      {accounts.length === 0 && !editing ? (
        <div style={{ textAlign: "center", padding: "50px 20px", color: "var(--text-6)" }}>
          ...
          <div style={{ fontSize: 14 }}>{t("smtp.noAccounts")}</div>
          <div style={{ fontSize: 12, color: "var(--text-7)", marginTop: 6 }}>{t("smtp.noAccountsHint")}</div>
        </div>
      ) : accounts.map(acc => (
        <div key={acc.id} style={{
          background: "var(--bg-card)", border: `1px solid ${editing?.id === acc.id ? "var(--accent-bd)" : "var(--border-2)"}`,
          ...
          <div style={{ width: 36, height: 36, borderRadius: 8, background: "var(--bg-hover)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <Icon name="server" size={16} style={{ color: acc.is_default ? "var(--accent)" : "var(--text-5)" }} />
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-2)", display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap" }}>
              {acc.name}
              {acc.is_default && (
                <span style={{ fontSize: 10, background: "var(--accent-bg)", color: "var(--accent)", padding: "2px 6px", borderRadius: 4 }}>{t("common.standard")}</span>
              )}
              {acc.match_domain && (
                <span style={{ fontSize: 10, background: "var(--blue-bg)", color: "var(--blue)", padding: "2px 7px", borderRadius: 4, fontFamily: "monospace" }}>@{acc.match_domain}</span>
              )}
            </div>
            <div style={{ fontSize: 11, color: "var(--text-6)", marginTop: 3, fontFamily: "monospace" }}>
              {acc.relay_user && <span style={{ color: "var(--green)" }}>{acc.relay_user}</span>}
              {acc.relay_user && <span style={{ color: "var(--text-7)" }}> → </span>}
              <span style={{ color: "var(--blue)" }}>{acc.relay_host || "—"}</span>
              {acc.relay_host && <span style={{ color: "var(--text-6)" }}>:{acc.relay_port}</span>}
            </div>
          </div>
